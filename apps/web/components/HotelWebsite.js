...
export default function HotelWebsite({ domain }) {
  ...
  // 💡 [신규] 객실 탭(폴더) 선택 상태 및 각 슬라이더 인덱스 상태
  const [selectedRoomId, setSelectedRoomId] = useState(null);
  const [roomSlideIdx, setRoomSlideIdx] = useState(0);
  const [contentSlideIdx, setContentSlideIdx] = useState(0);
  ...
  const sliderStyle = safeConfig.slider_style || 'fade'; // fade, zoom, slide

  const sliderImages = [];
  if (safeConfig.bg_image_url) sliderImages.push(safeConfig.bg_image_url);
  ...
  // 💡 서브 슬라이더 타이머 (객실, 부대시설, 관광지용)
  useEffect(() => {
    const timer = setInterval(() => {
        setRoomSlideIdx(prev => prev + 1);
        setContentSlideIdx(prev => (prev + 1) % sliderImages.length);
    }, 3000);
    return () => clearInterval(timer);
  }, [sliderImages.length]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col animate-fade-in custom-font selection:bg-slate-800 selection:text-white">
                    ...
                    <div className="bg-white rounded-b-3xl rounded-tr-3xl shadow-xl border border-slate-200 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-10 gap-8 relative z-0 -mt-px">
                        ...
                        <div className="lg:col-span-3 theme-bg-light p-6 md:p-8 rounded-3xl border theme-border flex flex-col justify-center h-full">
                            ...
                            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); alert("예약 시스템과 연결 중입니다."); }}>
                                <div>
                                    <label className="text-xs font-bold text-slate-600 uppercase">Check-in</label>
                                    <input type="date" className="w-full p-3 border border-white rounded-xl bg-white shadow-sm outline-none focus:ring-2 focus:ring-blue-400 font-bold text-slate-700" required />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-600 uppercase">Check-out</label>
                                    <input type="date" className="w-full p-3 border border-white rounded-xl bg-white shadow-sm outline-none focus:ring-2 focus:ring-blue-400 font-bold text-slate-700" required />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-600 uppercase">Guest Name</label>
                                    <input type="text" placeholder="John Doe" className="w-full p-3 border border-white rounded-xl bg-white shadow-sm outline-none focus:ring-2 focus:ring-blue-400 font-bold text-slate-700" required />
                                </div>
        ...
        {/* 🍴 메뉴 3: 부대시설 (FACILITIES) - 💡 [요청] 예약창 없이 70(사진):30(설명) 분할 */}
        {activeMenu === 'FACILITIES' && (
          <section className="pt-32 pb-20 px-6 max-w-7xl mx-auto animate-fade-in-up w-full flex-grow">
            <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-10 gap-8">
                {/* ⬅️ 왼쪽 70%: 갤러리 슬라이더 */}
                <div className="lg:col-span-7 w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden relative shadow-inner bg-black">
                    {sliderImages.map((img, idx) => (
                        <div key={idx} className={getSliderClass(idx, contentSlideIdx)}>
                    ...
                    <div className="absolute bottom-6 left-6 z-10 text-white drop-shadow-md">
                        ...
                        <h2 className="text-4xl font-black">Our Facilities</h2>
                    </div>
                </div>
                {/* ➡️ 오른쪽 30%: 상세 설명 */}
                <div className="lg:col-span-3 flex flex-col justify-center">
                    <h3 className="text-2xl font-black text-slate-800 mb-6 border-b-4 theme-border pb-2 inline-block">Services</h3>
                    <div className="whitespace-pre-wrap leading-loose text-slate-600 font-medium text-lg">
                      {safeConfig.facilities_json || "Information about facilities is currently being updated."}
                    </div>
                </div>
            </div>
          </section>
        ...
        {/* 🗺️ 메뉴 4: 주변 관광지 (ATTRACTIONS) - 70:30 (지도 제거, 사진 확장) */}
        {activeMenu === 'ATTRACTIONS' && (
          <section className="pt-32 pb-20 px-6 max-w-7xl mx-auto animate-fade-in-up w-full flex-grow">
             <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-10 gap-8">
                {/* ⬅️ 왼쪽 70%: 관광지 슬라이더 (높이 키움) */}
                <div className="lg:col-span-7 w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden relative shadow-inner bg-black">
                    {sliderImages.map((img, idx) => (
                        <div key={idx} className={getSliderClass(idx, contentSlideIdx)}>
                            <img src={img} className="w-full h-full object-cover opacity-80" />
                    ...
                    <div className="absolute bottom-6 left-6 z-10 text-white drop-shadow-md">
                        <div className="text-5xl mb-2">📸</div>
                        <h2 className="text-4xl font-black">Local Attractions</h2>
                    </div>
                </div>

                {/* ➡️ 오른쪽 30%: 관광지 상세 설명 */}
                <div className="lg:col-span-3 flex flex-col justify-center">
                    <h3 className="text-2xl font-black text-slate-800 mb-6 border-b-4 theme-border pb-2 inline-block">Explore Area</h3>
                    <div className="whitespace-pre-wrap leading-loose text-slate-600 font-medium text-lg">
                      {safeConfig.attractions_json || "Information about local attractions is currently being updated."}
                    </div>
                </div>
            </div>
          </section>
        ...
        {/* 📍 메뉴 5: 오시는 길 & 구글 지도 (LOCATION) - 분리 및 신설! */}
        {activeMenu === 'LOCATION' && (
          <section className="pt-32 pb-20 px-6 max-w-7xl mx-auto animate-fade-in-up w-full flex-grow">
             <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-10 gap-8">
                {/* ⬅️ 왼쪽 70%: 구글 지도 풀화면 */}
                ...
                <div className="lg:col-span-3 flex flex-col justify-center">
                    ...
                    <div className="space-y-4 text-slate-600">
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-100">
                            <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Need Help?</p>
                            <p className="font-bold text-slate-800">Contact our front desk for detailed directions.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        {/* 📱 💡 [수정] 헤더와 통일된 밝고 깨끗한 푸터 디자인 */}
        <footer className="bg-white/90 backdrop-blur-md border-t border-slate-200 py-10 px-6 text-center mt-auto">
          <div className="max-w-4xl mx-auto flex flex-col items-center gap-6">
              {(sns.ig || sns.fb) && (
                  <div className="flex gap-4 mb-2">
                      {sns.ig && (
                        <a href={sns.ig} target="_blank" rel="noreferrer" className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-pink-600 hover:bg-pink-600 hover:text-white transition-all shadow-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="20" height="20" x="2" y="2" rx="5" ry="5"/><path d="M16.11 7.99a.01.01 0 0 1 .02 0"/><path d="M15.82 12.18A4 4 0 1 1 11.82 8a4 4 0 0 1 4 4.18"/></svg>
                        </a>
                      )}
                      {sns.fb && (
                        <a href={sns.fb} target="_blank" rel="noreferrer" className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm">
              ...
              <p className="text-sm font-bold text-slate-500">
                  &copy; {new Date().getFullYear()} <span className="theme-text">{safeConfig.welcome_title || "Our Hotel"}</span>. All rights reserved.
              </p>
          </div>
        </footer>
      </div>
